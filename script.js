...
function entrarAlSistema(nombre) {
    ...
    sessionStorage.setItem('token_paolos', nombre);
}
...
window.addEventListener('load', () => {
    inicializarAdmin(); // <--- ESTO CREA TU USUARIO EN FIREBASE AL ABRIR
    
    const sesion = sessionStorage.getItem('token_paolos');
    if (sesion) {
...
function cerrarSesion() {
    sessionStorage.removeItem('token_paolos');
    location.reload();
}
//////////////////////////////
...
// --- ESTADO DE CAJA E HISTORIAL ---
let cajaAbierta = false;
let CajaActual = { base: 0, fecha: null, horaApertura: null };
...
function selectDestino(destino) {
    ...
    container.innerHTML = `
        ...
        <div class="categories-grid">
            ...
            <button class="category-btn" onclick="renderProductsByCategory('panzerotti', '${destino}')">🍔 PANZEROTTI</button>
...
function filterItems(dest) {
    ...
    if (searchTerm === "") { categoriesDiv.classList.remove('hidden'); container.innerHTML = ""; return; }
    ...
    for (const [catKey, items] of Object.entries(DB.menu)) {
        ...
        items.forEach(p => {
            if (p.nombre.toLowerCase().includes(searchTerm)) {
                found = true;
                if (catKey === 'lasañas') {
                    html += `<div class="product-card search-result"><h4>${p.nombre}</h4><div style="display:grid; gap:5px;">
                        ...
                        <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (F)', ${p.precio_f})">F: $${p.precio_f}</button></div></div>`;
                } else {
                    ...
                    html += `<div class="product-card search-result"><h4>${p.nombre}</h4><button class="btn-action" ${action}>AÑADIR</button></div>`;
                }
            }
        });
    }
    DB.bebidas_inv.forEach(p => {
        if (p.nombre.toLowerCase().includes(searchTerm)) {
            found = true;
            html += `<div class="product-card search-result"><h4>${p.nombre}</h4><button class="btn-action" onclick="sellBebida('${dest}', ${p.id})">AÑADIR</button></div>`;
        }
    });
    container.innerHTML = found ? html + `</div>` : `<div style="text-align:center; padding:20px;">Sin resultados.</div>`;
}
    ...
    if (cat === 'bebidas') {
        DB.bebidas_inv.forEach(p => {
            ...
            html += `<div class="product-card" style="${agotado ? 'opacity:0.6;' : ''}">
                ...
                <button class="btn-action" onclick="sellBebida('${dest}', ${p.id})" ${agotado ? 'disabled' : ''}>${agotado ? 'N/A' : 'AÑADIR'}</button></div>`;
        });
    } else {
        DB.menu[cat].forEach(p => {
            if (cat === 'pizzas_completa') {
                html += `<div class="product-card"><h4>${p.nombre}</h4><button class="btn-action" onclick="renderPizzaFlavorSelector('${dest}', '${p.nombre}', ${p.precio})">SABORES</button></div>`;
            } else if (cat === 'lasañas') {
                html += `<div class="product-card"><h4>${p.nombre}</h4><div style="display:grid; gap:5px;">
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (P)', ${p.precio_p})">P: $${p.precio_p}</button>
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (M)', ${p.precio_m})">M: $${p.precio_m}</button>
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (F)', ${p.precio_f})">F: $${p.precio_f}</button></div></div>`;
            } else if (cat === 'pastas') {
                html += `<div class="product-card"><h4>${p.nombre}</h4><div style="display:grid; gap:5px;">
                    ...
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (M)', ${p.precio_m})">M: $${p.precio_m}</button>
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (F)', ${p.precio_f})">F: $${p.precio_f}</button></div></div>`;
            } else if (cat === 'panzerotti') {
                html += `<div class="product-card"><h4>${p.nombre}</h4><div style="display:grid; gap:5px;">
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (P)', ${p.precio_p})">P: $${p.precio_p}</button>
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (M)', ${p.precio_m})">M: $${p.precio_m}</button>
                    <button class="category-btn" onclick="addItemToOrder('${dest}', '${p.nombre} (F)', ${p.precio_f})">F: $${p.precio_f}</button></div></div>`;
            } else {
                html += `<div class="product-card"><h4>${p.nombre}</h4><button class="btn-action" onclick="addItemToOrder('${dest}', '${p.nombre}', ${p.precio})">AÑADIR</button></div>`;
            }
        });
    }
...
function renderOrderSummary(dest) {
    ...
    html += `</div><div class="payment-selector">
            ...
            <button class="pay-btn ${metodoPagoSeleccionado === 'Transferencia' ? 'selected' : ''}" onclick="setMetodoPago('Transferencia')">📱 TRANSF.</button>
...
function setMetodoPago(m) {
    ...
    const title = document.getElementById('module-title').innerText.toLowerCase();
    renderOrderSummary(title);
}
...
function eliminarGasto(i) { Gastos.splice(i,1); renderOtros(document.getElementById('module-content')); }
...
function renderAjustes(container) {
    let html = `<div class="glass-card" style="padding:15px; border-top: 4px solid var(--accent);">
        <p style="font-size:0.8rem; color:var(--accent); margin-bottom:15px; text-align:center;">⚙️ PANEL DE CONTROL DE PRECIOS</p>`;

    html += `<h3 class="accent" style="margin-top:10px;">🍕 PRECIO PORCIONES</h3><table>`;
    DB.sabores_pizzas.forEach((s, idx) => {
        html += `<tr><td>${s.nombre}</td><td><input type="number" class="inv-input-inline" value="${s.precio}" onchange="DB.sabores_pizzas[${idx}].precio = parseInt(this.value); syncPrecios();"></td></tr>`;
    });
    html += `</table>`; // CERRAMOS TABLA DE PIZZAS
    // --- SECCIÓN: PIZZAS COMPLETAS ---
html += `<hr style="margin:25px 0; border:0; border-top:1px solid #444;">`;
html += `<h3 class="accent" style="margin-bottom:15px;">🥘 PIZZAS COMPLETAS</h3><table class="glass-table" style="width:100%;">`;

DB.menu.pizzas_completa.forEach((p, idx) => {
    html += `<tr>
        <td style="padding:10px 0;">${p.nombre}</td>
        <td style="text-align:right;">
            <input type="number" class="inv-input-inline" style="width:100px;" value="${p.precio}" onchange="DB.menu.pizzas_completa[${idx}].precio = parseInt(this.value); syncPrecios();">
        </td>
    </tr>`;
});
html += `</table>`; // CERRAMOS TABLA DE PIZZAS
...
// --- SECCIÓN: LASAÑAS ---
html += `<hr style="margin:25px 0; border:0; border-top:1px solid #444;">`;
html += `<h3 class="accent" style="margin-bottom:15px;">🍝 LASAÑAS (P / M / F)</h3><table class="glass-table" style="width:100%;">`;

DB.menu.lasañas.forEach((l, idx) => {
    html += `<tr style="border-bottom: 1px solid #222;">
        <td style="padding:15px 0;">${l.nombre}</td>
        <td style="display:flex; gap:8px; justify-content:flex-end; padding:10px 0;">
            <div style="text-align:center;">
                <small style="color:gray;">P</small><br>
                <input type="number" class="inv-input-inline" style="width:65px;" value="${l.precio_p}" onchange="DB.menu.lasañas[${idx}].precio_p = parseInt(this.value); syncPrecios();">
            </div>
            <div style="text-align:center;">
                <small style="color:gray;">M</small><br>
                <input type="number" class="inv-input-inline" style="width:65px;" value="${l.precio_m}" onchange="DB.menu.lasañas[${idx}].precio_m = parseInt(this.value); syncPrecios();">
            </div>
            <div style="text-align:center;">
                <small style="color:gray;">F</small><br>
                <input type="number" class="inv-input-inline" style="width:65px;" value="${l.precio_f}" onchange="DB.menu.lasañas[${idx}].precio_f = parseInt(this.value); syncPrecios();">
            </div>
        </td>
    </tr>`;
});
html += `</table>`; // CERRAMOS TABLA DE LASAÑAS
    html += `</table><hr style="margin:20px 0; border:0; border-top:1px solid #333;">`;

    html += `<h3 class="accent">🥞 CREPES</h3><table>`;
    DB.menu.crepes.forEach((c, idx) => {
        html += `<tr><td>${c.nombre}</td><td><input type="number" class="inv-input-inline" value="${c.precio}" onchange="DB.menu.crepes[${idx}].precio = parseInt(this.value); syncPrecios();"></td></tr>`;
    });
